...
public final class MultiGetMap<K, V> implements Store<K, V> {
    ...
    private final void waitTillEmpty() {
        ...
        if (!scheduled.isEmpty() || processing.get() > 0) {
            // waitTillEmpty();
            throw new RuntimeException("Multi get map could not be emptied. Items still processing: " + processing.get()
    ...
    private final class ProcessGets implements Runnable {
        ...
        public void run() {
            ...
            /*
             * if (toProcessKeys.size() > 1) { System.out.println(
             * "do multi get " + toProcessKeys); }
             */
